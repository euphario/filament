...
use libos::mmio::{MmioRegion, DmaPool, delay_us, cache_clean, cache_invalidate};
...
use libos::ring::{IoSqe, IoCqe, io_op, io_status, side_msg, side_status, SideEntry};
use libos::{uinfo, unotice, udebug, uerror, uwarn};
...
// Network opcodes: use io_op::NET_SEND from libos::ring (already imported)
...
impl EthDriver {
    ...
    fn reset_switch(&self) {
        let ethwarp = match &self.ethwarp {
            Some(e) => e,
            None => {
                udebug!("ethd", "reset_switch_skip"; reason = "no_ethwarp");
                return;
            }
        };

        udebug!("ethd", "reset_switch_start";);

        // Read current reset state
        let rst = ethwarp.read32(ETHWARP_RST_OFS);
        udebug!("ethd", "ethwarp_rst_before"; val = libos::ulog::hex32(rst));
        ...
        udebug!("ethd", "reset_switch_done"; val = libos::ulog::hex32(rst_after));
    }
    ...
    fn pinctrl_led_mux(&self) {
        ...
        udebug!("ethd", "pinctrl_led_mux"; reg_ofs = libos::ulog::hex32(PINCTRL_LED_REG_OFS as u32),
               old = libos::ulog::hex32(old_val), new = libos::ulog::hex32(new_val),
               verify = libos::ulog::hex32(verify));
    }
    ...
    fn phy_led_init(&self) {
        ...
        for phy in 0..4u8 {
            ...
            udebug!("ethd", "phy_led_init"; phy = phy, old = libos::ulog::hex32(old_led0 as u32),
                   new = libos::ulog::hex32(new_led0 as u32));
        }
    ...
    fn switch_init(&mut self) {
        udebug!("ethd", "switch_init_start";);

        // Check if switch MMIO was mapped
        if self.sw.is_none() {
            udebug!("ethd", "switch_init_skipped"; reason = "no_mmio");
            return;
        }
        ...
        udebug!("ethd", "switch_regs"; sys = libos::ulog::hex32(sys_ctrl), pll0 = libos::ulog::hex32(pll_7500),
               pll1 = libos::ulog::hex32(pll_7504), clk = libos::ulog::hex32(clk_7508), misc = libos::ulog::hex32(misc_7804));
        ...
        udebug!("ethd", "mac_misc_v3_uboot"; val = libos::ulog::hex32(mac_misc),
               mux_to_esw = if mac_misc & 1 != 0 { 1u32 } else { 0u32 },
               mdc_turbo = if mac_misc & 0x10 != 0 { 1u32 } else { 0u32 });
        udebug!("ethd", "ppsc_uboot"; val = libos::ulog::hex32(ppsc));
        ...
        for phy in 0..4u8 {
            ...
            udebug!("ethd", "phy"; addr = phy, id1 = libos::ulog::hex32(id1 as u32), id2 = libos::ulog::hex32(id2 as u32),
                   status = libos::ulog::hex32(status as u32), link = if link { 1u32 } else { 0u32 });

            // Read current BMCR
            let bmcr = self.mdio_read(phy, BMCR).unwrap_or(0);
            udebug!("ethd", "phy_bmcr"; addr = phy, bmcr = libos::ulog::hex32(bmcr as u32),
            ...
            if (bmcr & BMCR_POWER_DOWN) != 0 {
                udebug!("ethd", "phy_power_up"; addr = phy);
            ...
            udebug!("ethd", "phy_autoneg_restart"; addr = phy);
        }
        ...
        for phy in 0..4u8 {
            ...
            udebug!("ethd", "phy_status"; addr = phy, link = if link { 1u32 } else { 0u32 },
        ...
        udebug!("ethd", "cpu_port_uboot"; pcr = libos::ulog::hex32(cpu_pcr), pmcr = libos::ulog::hex32(cpu_pmcr));

        for port in 0..4 {
            let pcr = self.gsw_read(MT7530_PCR_P(port));
            let pmcr = self.gsw_read(MT7530_PMCR_P(port));
            udebug!("ethd", "user_port_uboot"; port = port, pcr = libos::ulog::hex32(pcr), pmcr = libos::ulog::hex32(pmcr));
        }
        ...
        udebug!("ethd", "cpu_port_config_start";);
        ...
        udebug!("ethd", "mfc_configured"; before = libos::ulog::hex32(mfc_before),
               after = libos::ulog::hex32(mfc_after));
        ...
        udebug!("ethd", "cfc_configured"; before = libos::ulog::hex32(cfc_before),
               after = libos::ulog::hex32(cfc_after));
        ...
        udebug!("ethd", "pvc_configured"; before = libos::ulog::hex32(pvc_before),
               after = libos::ulog::hex32(pvc_after));
        ...
        udebug!("ethd", "cpu_pcr_configured"; val = libos::ulog::hex32(pcr_val),
               readback = libos::ulog::hex32(pcr_after));
        ...
        udebug!("ethd", "cpu_pmcr_configured"; before = libos::ulog::hex32(pmcr_before),
               after = libos::ulog::hex32(pmcr_after),
        ...
        for port in 0..4u32 {
            ...
            udebug!("ethd", "user_pcr_configured"; port = port,
                   before = libos::ulog::hex32(port_pcr),
                   after = libos::ulog::hex32(port_pcr));
        }
        ...
        udebug!("ethd", "switch_led_before"; led_en = libos::ulog::hex32(led_en_before),
               led_io = libos::ulog::hex32(led_io_before));
        ...
        udebug!("ethd", "switch_led_after"; led_en = libos::ulog::hex32(led_en_after),
               led_io = libos::ulog::hex32(led_io_after));
    ...
    fn fifo_init(&mut self) {
        ...
        if use_sram {
            ...
            udebug!("ethd", "fifo_init_sram";
                   sram_vaddr = libos::ulog::hex64(self.sram_vaddr),
                   sram_paddr = libos::ulog::hex64(self.sram_paddr),
                   buf_paddr = libos::ulog::hex64(self.buf_paddr));
        } else {
            ...
            udebug!("ethd", "fifo_init_dma";
                   tx_ring_paddr = libos::ulog::hex64(tx_ring_paddr),
                   rx_ring_paddr = libos::ulog::hex64(rx_ring_paddr),
                   buf_paddr = libos::ulog::hex64(self.buf_paddr));
        }
        ...
        udebug!("ethd", "pdma_stop_start"; glo = libos::ulog::hex32(glo_before));
        ...
        udebug!("ethd", "pdma_stopped"; glo = libos::ulog::hex32(glo_after));
        ...
        if USE_QDMA {
            ...
            udebug!("ethd", "fifo_init_qdma"; mode = "qdma_tx_pdma_rx");
        } else {
        ...
        udebug!("ethd", "fifo_init_done";
               tx_ring = libos::ulog::hex64(tx_ring_paddr),
               rx_ring = libos::ulog::hex64(rx_ring_paddr));

        if DEBUG_VERBOSE {
            udebug!("ethd", "verify_tx";
                   base = libos::ulog::hex32(verify_tx_base),
                   max = verify_tx_max,
                   ctx = verify_tx_ctx,
                   dtx = verify_tx_dtx);
            udebug!("ethd", "verify_rx";
                   base = libos::ulog::hex32(verify_rx_base),
                   max = verify_rx_max);
            ...
            unsafe {
                ...
                udebug!("ethd", "verify_tx0";
                       txd1 = libos::ulog::hex32(txd1),
                       txd2 = libos::ulog::hex32(txd2),
                       txd3 = libos::ulog::hex32(txd3),
                       txd5 = libos::ulog::hex32(txd5));
                ...
                udebug!("ethd", "verify_rx0";
                       rxd1 = libos::ulog::hex32(rxd1),
                       rxd2 = libos::ulog::hex32(rxd2));
            }
        }
    }
    ...
    fn qdma_stop(&mut self) {
        ...
        udebug!("ethd", "qdma_stop_start"; glo = libos::ulog::hex32(glo_before));
        ...
        udebug!("ethd", "qdma_stopped"; glo = libos::ulog::hex32(glo_after));
    }
    ...
    fn qdma_init_fq(&mut self) {
        ...
        udebug!("ethd", "qdma_fq_init";
               fq_head = libos::ulog::hex64(fq_ring_paddr),
               fq_tail = libos::ulog::hex64(fq_tail_paddr),
    ...
    fn qdma_init_tx_ring(&mut self) {
        ...
        udebug!("ethd", "qdma_tx_init";
               tx_ring = libos::ulog::hex64(tx_ring_paddr),
               ctx_ptr = libos::ulog::hex32(tx_ring_paddr as u32),
    ...
    fn qdma_start(&mut self) {
        ...
        let glo = self.qdma_read(QDMA_GLO_CFG);
        udebug!("ethd", "qdma_start_before"; glo = libos::ulog::hex32(glo));
        ...
        let glo_after = self.qdma_read(QDMA_GLO_CFG);
        udebug!("ethd", "qdma_start_after"; glo = libos::ulog::hex32(glo_after));
    }
    ...
    fn qdma_send(&mut self, packet: &[u8]) -> bool {
        ...
        unsafe {
            ...
            if (txd3 & QDMA_TX_DMA_OWNER_CPU) == 0 {
                ...
                udebug!("ethd", "qdma_tx_full"; idx = self.tx_idx as u32);
                return false;
            }
        ...
        udebug!("ethd", "qdma_tx_sent"; idx = self.tx_idx as u32, len = packet.len() as u32,
               dtx = libos::ulog::hex32(dtx), ctx = libos::ulog::hex32(ctx),
               txd1 = libos::ulog::hex32(txd1), txd2 = libos::ulog::hex32(txd2),
               txd3 = libos::ulog::hex32(txd3), txd4 = libos::ulog::hex32(txd4));
    ...
    fn dump_all_registers(&self, prefix: &str) {
        ...
        udebug!("ethd", "dump_fe_glo"; prefix = prefix, glo_misc = libos::ulog::hex32(fe_glo_misc),
               glo_cfg = libos::ulog::hex32(fe_glo_cfg));
        ...
        udebug!("ethd", "dump_pse"; prefix = prefix, fqfc = libos::ulog::hex32(pse_fqfc),
               iqfc = libos::ulog::hex32(pse_iqfc), drop = libos::ulog::hex32(pse_drop),
               dumy = libos::ulog::hex32(pse_dumy));
        udebug!("ethd", "dump_pse2"; prefix = prefix, buf_ctrl = libos::ulog::hex32(pse_buf_ctrl));

        // PSE IQ_REV registers (free buffer thresholds per port)
        for i in 0..9u32 {
            let iq_rev = self.fe_read(0x118 + i * 4);
            if iq_rev != 0 {
                udebug!("ethd", "dump_pse_iq"; prefix = prefix, idx = i, val = libos::ulog::hex32(iq_rev));
            }
        }
        ...
        udebug!("ethd", "dump_pse_oq"; prefix = prefix, oq0 = libos::ulog::hex32(pse_oq0),
               oq1 = libos::ulog::hex32(pse_oq1), oq2 = libos::ulog::hex32(pse_oq2));
        ...
        for gdm in 0..3u32 {
            ...
            udebug!("ethd", "dump_gdm"; prefix = prefix, gdm = gdm, ig = libos::ulog::hex32(ig),
                   eg = libos::ulog::hex32(eg), fwd = libos::ulog::hex32(fwd), shp = libos::ulog::hex32(shp));
        }
        ...
        for port in 0..3u32 {
            ...
            udebug!("ethd", "dump_gmac"; prefix = prefix, port = port, mcr = libos::ulog::hex32(mcr));
        }
        ...
        let ppsc = self.fe.as_ref().map(|fe| fe.read32(GMAC_BASE as usize + 0x00)).unwrap_or(0);
        udebug!("ethd", "dump_mac_misc"; prefix = prefix, mac_misc = libos::ulog::hex32(mac_misc_v3),
               ppsc = libos::ulog::hex32(ppsc));

        // FE reset and clock state
        let fe_rst = self.fe_read(0x04);  // FE_RST_GLO
        let fe_crc = self.fe_read(0x20);  // FE CRC config
        udebug!("ethd", "dump_fe_rst"; prefix = prefix, rst = libos::ulog::hex32(fe_rst),
               crc = libos::ulog::hex32(fe_crc));
        ...
        udebug!("ethd", "dump_path"; prefix = prefix, mac_misc_fe = libos::ulog::hex32(mac_misc_fe));
        ...
        udebug!("ethd", "dump_xgmac"; prefix = prefix,
               sts = libos::ulog::hex32(xgmac_sts),
               gsw_cfg = libos::ulog::hex32(gsw_cfg),
        ...
        udebug!("ethd", "dump_gdm_stats"; prefix = prefix,
        ...
        if let Some(ew) = &self.ethwarp {
            ...
            udebug!("ethd", "dump_ethwarp"; prefix = prefix, rst = libos::ulog::hex32(ew_rst));
        }
        ...
        udebug!("ethd", "dump_pdma"; prefix = prefix, glo = libos::ulog::hex32(pdma_glo),
               rst = libos::ulog::hex32(pdma_rst), int_sta = libos::ulog::hex32(pdma_int_sta),
               int_en = libos::ulog::hex32(pdma_int_en));
        ...
        udebug!("ethd", "dump_tx_ring"; prefix = prefix, base = libos::ulog::hex32(tx_base),
        ...
        udebug!("ethd", "dump_qdma"; prefix = prefix, glo = libos::ulog::hex32(qdma_glo),
               tx_base = libos::ulog::hex32(qdma_tx_base), tx_max = qdma_tx_max,
        ...
        if self.sw.is_some() {
            ...
            udebug!("ethd", "dump_sw_cpu"; prefix = prefix, pcr = libos::ulog::hex32(cpu_pcr),
                   pmcr = libos::ulog::hex32(cpu_pmcr), pvc = libos::ulog::hex32(cpu_pvc));
            ...
            for port in 0..4u32 {
                ...
                udebug!("ethd", "dump_sw_port"; prefix = prefix, port = port,
                       pcr = libos::ulog::hex32(pcr), pmcr = libos::ulog::hex32(pmcr));
            }
            ...
            udebug!("ethd", "dump_sw_sys"; prefix = prefix, sys = libos::ulog::hex32(sys_ctrl),
                   mfc = libos::ulog::hex32(mfc), pfc = libos::ulog::hex32(pfc));
        }
    }
    ...
    fn eth_start(&mut self) {
        ...
        if let Some(ref ethsys) = self.ethsys {
            ...
            udebug!("ethd", "fe_reset_skipped"; rst_state = libos::ulog::hex32(rst_state),
        ...
        udebug!("ethd", "gdm1_0x500_configured"; ig = libos::ulog::hex32(gdm1_ig),
               eg = libos::ulog::hex32(gdm1_eg));

        let gdm2_ig = self.fe_read(GDMA2_BASE + GDMA_IG_CTRL_REG);  // GDM2 at 0x1500
        let gdm2_eg = self.fe_read(GDMA2_BASE + GDMA_EG_CTRL_REG);
        udebug!("ethd", "gdm2_0x1500_configured"; ig = libos::ulog::hex32(gdm2_ig),
               eg = libos::ulog::hex32(gdm2_eg));
        ...
        if self.sw.is_some() {
            let cpu_pcr = self.gsw_read(MT7530_PCR_P(MT7531_CPU_PORT));
            let cpu_pmcr = self.gsw_read(MT7530_PMCR_P(MT7531_CPU_PORT));
            udebug!("ethd", "switch_cpu_port"; pcr = libos::ulog::hex32(cpu_pcr), pmcr = libos::ulog::hex32(cpu_pmcr));
        }
        ...
        //   mtk_gmac_write(eth_priv, gmac_port_mcr(eth_priv.gmac_id), FORCE_MODE);
        {
            ...
            udebug!("ethd", "gmac0_mcr_before"; mcr = libos::ulog::hex32(mcr0));
            ...
            udebug!("ethd", "gmac0_mcr_after"; mcr = libos::ulog::hex32(mcr0_after),
            ...
            udebug!("ethd", "xgmac_sts"; before = libos::ulog::hex32(xgmac_before),
                   after = libos::ulog::hex32(xgmac_after),
            ...
            udebug!("ethd", "gsw_cfg"; before = libos::ulog::hex32(gsw_before),
                   after = libos::ulog::hex32(gsw_after));
        }

        for i in 1..3u32 {
            let mcr = self.fe.as_ref().map(|fe| fe.read32(GMAC_BASE as usize + gmac_port_mcr(i) as usize)).unwrap_or(0);
            udebug!("ethd", "gmac_mcr_other"; port = i, mcr = libos::ulog::hex32(mcr));
        }
        ...
        if USE_QDMA {
            ...
            udebug!("ethd", "dma_enabled_qdma";
                   pdma_glo = libos::ulog::hex32(pdma_glo),
                   qdma_glo = libos::ulog::hex32(qdma_glo));
        } else {
            ...
            udebug!("ethd", "dma_enabled"; glo_cfg = libos::ulog::hex32(glo_cfg));
        }

        // LAST: Dump state AFTER all our changes
        if DEBUG_VERBOSE {
            udebug!("ethd", "dump_our_state";);
    ...
    fn eth_send(&mut self, packet: &[u8]) -> bool {
        ...
        unsafe {
            ...
            if (txd2 & PDMA_TXD2_DDONE) == 0 {
                udebug!("ethd", "tx_full"; idx = self.tx_idx as u32);
                return false;
            }
        ...
        udebug!("ethd", "tx_sent"; idx = self.tx_idx as u32, len = packet.len() as u32, dtx = dtx, ctx = ctx);
    ...
    fn process_tx(&mut self, ctx: &mut dyn BusCtx) {
        let port_id = match self.port_id {
            Some(id) => id,
            None => {
                udebug!("ethd", "process_tx_no_port";);
                return;
            }
        };
        ...
        for _ in 0..8 {
            ...
            count += 1;
            udebug!("ethd", "tx_request"; opcode = sqe.opcode, tag = sqe.tag, len = sqe.data_len);
        ...
        if count > 0 {
            udebug!("ethd", "process_tx_done"; count = count);
        }
    }
            ...
            if let Some(entry) = queries[i].take() {
                match entry.msg_type {
                    side_msg::QUERY_INFO => {
                        ...
                        udebug!("ethd", "side_query"; msg = "info_response");
                    }
...
impl Driver for EthDriver {
    fn reset(&mut self, ctx: &mut dyn BusCtx) -> Result<(), BusError> {
        ...
        // Test using FE_GLO_MISC at offset 0x124
        {
            ...
            udebug!("ethd", "fe_glo_misc_uboot"; val = libos::ulog::hex32(glo_misc),
            ...
            if (glo_misc & PDMA_VER_V2) == 0 {
                ...
                udebug!("ethd", "fe_glo_misc_after"; val = libos::ulog::hex32(glo_misc_after),
            ...
            udebug!("ethd", "fe_write_test"; reg = "pse_dumy", before = libos::ulog::hex32(pse_dumy_before),
                   wrote = libos::ulog::hex32(0x12345678), after = libos::ulog::hex32(pse_dumy_after));
        ...
        match MmioRegion::open(SWITCH_BASE, SWITCH_SIZE) {
            Some(sw) => {
                udebug!("ethd", "switch_mmio_ok"; base = libos::ulog::hex64(SWITCH_BASE));
                self.sw = Some(sw);
            }
            None => {
                uwarn!("ethd", "switch_mmio_failed"; base = libos::ulog::hex64(SWITCH_BASE));
        ...
        match MmioRegion::open(ETHWARP_BASE, ETHWARP_SIZE) {
            Some(ew) => {
                udebug!("ethd", "ethwarp_mmio_ok"; base = libos::ulog::hex64(ETHWARP_BASE));
                self.ethwarp = Some(ew);
            }
            None => {
                uwarn!("ethd", "ethwarp_mmio_failed"; base = libos::ulog::hex64(ETHWARP_BASE));
            }
        }
        ...
        match MmioRegion::open(ETHSYS_BASE, ETHSYS_SIZE) {
            Some(es) => {
                udebug!("ethd", "ethsys_mmio_ok"; base = libos::ulog::hex64(ETHSYS_BASE));

                // Read current clock gate status
                let clk_gate_before = es.read32(ETHSYS_CLK_GATE_REG);
                udebug!("ethd", "ethsys_clk_gate_before"; val = libos::ulog::hex32(clk_gate_before));
                ...
                udebug!("ethd", "ethsys_clk_gate_after"; val = libos::ulog::hex32(clk_gate_after),
                ...
                udebug!("ethd", "ethsys_regs"; dummy = libos::ulog::hex32(dummy_reg),
                       syscfg = libos::ulog::hex32(sys_cfg), syscfg0 = libos::ulog::hex32(sys_cfg0));

                self.ethsys = Some(es);
            }
            None => {
                uwarn!("ethd", "ethsys_mmio_failed"; base = libos::ulog::hex64(ETHSYS_BASE));
            }
        }
        ...
        match MmioRegion::open(PINCTRL_BASE, PINCTRL_SIZE) {
            Some(pc) => {
                udebug!("ethd", "pinctrl_mmio_ok"; base = libos::ulog::hex64(PINCTRL_BASE));
                self.pinctrl = Some(pc);
            }
            None => {
                uwarn!("ethd", "pinctrl_mmio_failed"; base = libos::ulog::hex64(PINCTRL_BASE));
            }
        }
            ...
            match MmioRegion::open(SRAM_BASE, SRAM_SIZE) {
                Some(sr) => {
                    ...
                    if test_val == 0xCAFEBABE {
                        udebug!("ethd", "sram_ok"; base = libos::ulog::hex64(SRAM_BASE),
                               size = libos::ulog::hex64(SRAM_SIZE));
                        self.sram_vaddr = sr.virt_base();
                        self.sram_paddr = SRAM_BASE;
                        // Keep the mapping alive
                        core::mem::forget(sr);
                    } else {
                        uerror!("ethd", "sram_write_failed"; expected = libos::ulog::hex32(0xCAFEBABE),
                                got = libos::ulog::hex32(test_val));
                        // Fall back to DMA memory
                    }
                }
                None => {
                    uerror!("ethd", "sram_mmio_failed"; base = libos::ulog::hex64(SRAM_BASE));
                }
            }
        }
        ...
        udebug!("ethd", "sram_config";
               ...
               paddr = libos::ulog::hex64(if self.sram_vaddr != 0 { self.sram_paddr } else { 0 }));
        ...
        let mut dma = if USE_STREAMING_DMA {
            udebug!("ethd", "dma_mode"; mode = "streaming");
            DmaPool::alloc_streaming(DMA_POOL_SIZE)
        } else {
            udebug!("ethd", "dma_mode"; mode = "coherent");
        ...
        if tag == self.poll_tag {
            ...
            if DEBUG_VERBOSE && self.poll_count % 100 == 0 {
                if USE_QDMA {
                    ...
                    udebug!("ethd", "qdma_status"; ctx = libos::ulog::hex32(ctx), dtx = libos::ulog::hex32(dtx),
                           glo = libos::ulog::hex32(glo), fq_cnt = libos::ulog::hex32(fq_cnt), poll = self.poll_count);
                    ...
                    udebug!("ethd", "pdma_rx_status"; glo = libos::ulog::hex32(pdma_glo),
                    ...
                    udebug!("ethd", "tx_status"; dtx = dtx, ctx = ctx_idx, ddone_mask = libos::ulog::hex32(ddone_bits), poll = self.poll_count);
                } else {
                    // Regular DMA mode (no SRAM, no QDMA)
                    let pdma_glo = self.pdma_read(PDMA_GLO_CFG_REG);
                    let dtx = self.pdma_read(tx_dtx_idx_reg(0));
                    let ctx_idx = self.pdma_read(tx_ctx_idx_reg(0));
                    udebug!("ethd", "pdma_status"; glo = libos::ulog::hex32(pdma_glo),
                ...
                for gmac in 0..3u32 {
                    ...
                    udebug!("ethd", "gdm_cnt"; gmac = gmac, rx_b = rx_bytes, rx_p = rx_pkts,
                ...
                for gmac in 0..3u32 {
                    ...
                    udebug!("ethd", "gmac_mcr"; port = gmac, mcr = libos::ulog::hex32(mcr));
                }
                ...
                udebug!("ethd", "pdma_state"; glo = libos::ulog::hex32(pdma_glo),
                       tx_ptr = libos::ulog::hex32(pdma_tx_ptr), ctx = pdma_tx_ctx, dtx = pdma_tx_dtx);

                udebug!("ethd", "fe_status"; pse_fq = libos::ulog::hex32(pse_fq), pse_drop = libos::ulog::hex32(pse_drop),
                       pse_oq = libos::ulog::hex32(pse_oqc));
            }
    ...
    fn config_keys(&self) -> &[libos::bus::ConfigKey] {
