//! Collections — mirrors `std::collections`.
//!
//! Re-exports alloc collections and provides `HashMap` with FxHash
//! (fast, non-cryptographic, no entropy needed).

extern crate alloc;

// Re-exports from alloc
pub use alloc::collections::{BTreeMap, BTreeSet, BinaryHeap, VecDeque};
...
pub mod hash_map {
    ...
    impl<K, V> Slot<K, V> {
        ...
        fn is_searchable(&self) -> bool {
            // Search continues through Tombstones, stops at Empty
            !matches!(self, Slot::Empty)
        }
    }
    ...
    mod tests {
        ...
        fn remove() {
            ...
            assert_eq!(map.get(&1), None);
            assert_eq!(map.len(), 1);
            // Can still find key 2 past the tombstone
        ...
        fn iter() {
            ...
            map.insert(1u32, 10u32);
            map.insert(2, 20);
            let mut entries: alloc::vec::Vec<_> = map.iter().map(|(&k, &v)| (k, v)).collect();
            entries.sort();
            assert_eq!(entries, alloc::vec![(1, 10), (2, 20)]);
        }

        #[test]
        fn from_iterator() {
            let map: HashMap<u32, u32> = [(1, 10), (2, 20), (3, 30)].into_iter().collect();
            assert_eq!(map.len(), 3);
            assert_eq!(map.get(&2), Some(&20));
        }
        ...
        fn string_keys() {
            ...
            let mut map = HashMap::new();
            map.insert(String::from("hello"), 1u32);
