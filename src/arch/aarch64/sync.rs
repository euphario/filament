...
/// ```
pub struct IrqGuard {
    /// Saved DAIF value to restore on drop
    saved_daif: u64,
}

impl IrqGuard {
    /// Create a new IrqGuard, disabling IRQs.
    /// The previous interrupt state is saved and will be restored on drop.
    #[inline]
    pub fn new() -> Self {
        let saved_daif: u64;
        unsafe {
            // Read current DAIF
            core::arch::asm!("mrs {}, daif", out(reg) saved_daif);
    ...
    pub fn was_enabled(&self) -> bool {
        // I bit is bit 7 of DAIF - if clear, IRQs were enabled
        (self.saved_daif & (1 << 7)) == 0
    }
}
...
mod tests {
    ...
    pub fn test() {
        ...
        assert_eq!(before, after, "DAIF not restored after guard drop");
